...
DECLARE
  caller_rolle TEXT;
BEGIN
  SELECT rolle INTO caller_rolle FROM benutzer WHERE id = current_setting('request.jwt.claims', true)::json->>'user_id';
  IF caller_rolle != 'admin' THEN
    RAISE EXCEPTION 'Nur Admins dürfen Benutzer anlegen' USING ERRCODE = '42501';
  END IF;
  INSERT INTO benutzer (email, name, rolle, telefon, passwort_hash, aktiv)
  VALUES (
    NEW.email,
    NEW.name,
...
BEGIN
  ...
  END IF;
  DELETE FROM benutzer WHERE id = OLD.id;
  RETURN OLD;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
